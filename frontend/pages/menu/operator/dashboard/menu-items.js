  ...
  return (
    <Box>
      <Flex justifyContent="space-between" alignItems="center" mb={4}>
        ...
        <Button onClick={() => router.push('/menu/operator/dashboard/menu-items/create')} colorScheme="green">
          Add Menu Item
        </Button>
      </Flex>
          ...
          {menuItems.map((item) => (
            <Box key={item.id} p={4} borderWidth="1px" borderRadius="md">
              <Flex justify="space-between">
                ...
                <Flex gap={2}>
                  <Button colorScheme="blue" size="xs" onClick={() => router.push(`/menu/operator/dashboard/menu-items/edit?id=${item.id}`)}>Edit</Button>
