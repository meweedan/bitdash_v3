...
const OrderDetails = ({ order, onClose, onUpdateStatus, operatorData }) => {
  ...
  // Helper functions to fix table name and order display issues
const getTableName = (order) => {
  if (!order) return 'N/A';
  
  // Check if it's an array of tables with data structure
  if (order.attributes?.tables?.data && order.attributes.tables.data.length > 0) {
    return order.attributes.tables.data[0].attributes?.name || 'N/A';
  }
  
  // Check if it's a direct tables array
  if (order.tables && order.tables.length > 0) {
    return order.tables[0].name || 'N/A';
  }
  
  return 'N/A';
};

// Helper to safely extract customer info
const getCustomerInfo = (order) => {
  ...
  if (order.attributes?.customer_profile?.data?.attributes) {
    info.name = order.attributes.customer_profile.data.attributes.fullName || info.name;
    info.phone = order.attributes.customer_profile.data.attributes.phone || info.phone;
    info.address = order.attributes.customer_profile.data.attributes.address || info.address;
  } 
  // Check for guest info
  else if (order.attributes?.guest_info) {
    info.name = order.attributes.guest_info.name || info.name;
    info.phone = order.attributes.guest_info.phone || info.phone;
    info.address = order.attributes.guest_info.address || info.address;
  }
...
const filteredOrders = () => {
  ...
  if (orderFilter === 'active') {
    ...
    filtered = filtered.filter(order => order.attributes?.status === 'completed');
  } else if (orderFilter === 'cancelled') {
    filtered = filtered.filter(order => order.attributes?.status === 'cancelled');
  }
...
const normalizeOrderData = (orders) => {
  ...
  // Add this helper function to properly extract table name
const getTableName = (order) => {
  if (!order) return 'N/A';
          ...
          >
            <VStack spacing={8} align="stretch">
              ...
              <SimpleGrid columns={{ base: 1, md: 4 }} spacing={6} mb={6}>
                <DashboardCard p={4}>
                  <VStack align="stretch">
                    <HStack justify="space-between">
                      <Text color="gray.500">{t('activeOrders')}</Text>
                      <Icon as={FiActivity} color="red.500" />
                    </HStack>
                    <Text fontSize="2xl" fontWeight="bold">
                      {orders.filter(o => ['pending', 'preparing', 'ready'].includes(o.attributes?.status)).length}
                    </Text>
                  </VStack>
                </DashboardCard>
                <DashboardCard p={4}>
                  <VStack align="stretch">
                    <HStack justify="space-between">
                      <Text color="gray.500">{t('todaysOrders')}</Text>
                ...
                <DashboardCard p={4}>
                  <VStack align="stretch">
                    <HStack justify="space-between">
                      <Text color="gray.500">{t('completedToday')}</Text>
                ...
                <DashboardCard p={4}>
                  <VStack align="stretch">
                    <HStack justify="space-between">
                      <Text color="gray.500">{t('totalRevenue')}</Text>
                      <Icon as={FiDollarSign} color="purple.500" />
                    </HStack>
                    <Text fontSize="2xl" fontWeight="bold">
                      ${orders.reduce((sum, order) => sum + parseFloat(order.attributes?.total || 0), 0).toFixed(2)}
                    </Text>
                  </VStack>
                </DashboardCard>
              </SimpleGrid>

              {/* Add Pickup/Delivery Section */}
              {userData?.restaurant && (
              ...
              <Tabs isFitted variant="enclosed">
                ...
                <TabPanels>
                  {/* Tables Tab - Filter out pickup/delivery tables */}
                  <TabPanel>
                    <VStack spacing={4} align="stretch">
                      <Flex justify="space-between" align="center">
                        <Heading size="md">
                      ...
                      {!userData?.restaurant ? (
                        ...
                        <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={6}>
                          {userData.restaurant.tables
                            .filter(table => {
                              const tableName = (table.attributes?.name || table.name || '').toLowerCase();
                              // Filter out pickup and delivery tables
                              return !['pickup', 'استلام', 'delivery', 'توصيل'].includes(tableName);
                            })
                            .map((table) => (
                              <TableCard
                                key={table.id}
          ...
          {selectedOrder && (
            <VStack spacing={4} align="stretch">
              ...
              <Box>
                ...
                {selectedOrder.attributes?.customer_profile?.data?.attributes ? (
                  <>
                    <Text>Name: {selectedOrder.attributes.customer_profile.data.attributes.fullName || "Guest"}</Text>
                    <Text>Phone: {selectedOrder.attributes.customer_profile.data.attributes.phone || "N/A"}</Text>
                    {selectedOrder.attributes.customer_profile.data.attributes.address && (
                      <Text>Address: {selectedOrder.attributes.customer_profile.data.attributes.address}</Text>
                    )}
                  </>
                ) : selectedOrder.attributes?.guest_info ? (
                  <>
                    <Text>Name: {selectedOrder.attributes.guest_info.name || "Guest"}</Text>
                    <Text>Phone: {selectedOrder.attributes.guest_info.phone || "N/A"}</Text>
                    {selectedOrder.attributes.guest_info.address && (
                      <Text>Address: {selectedOrder.attributes.guest_info.address}</Text>
                    )}
                  </>
                ) : (
              ...
              <Box>
                <Text fontWeight="bold">Table:</Text>
                <Text>{getTableName(selectedOrder)}</Text>
              </Box>

              {/* Order Type if present */}
              {selectedOrder.attributes?.orderType && (
                <Box>
                  <Text fontWeight="bold">Order Type:</Text>
                  <Text>{selectedOrder.attributes.orderType}</Text>
                </Box>
              )}

              {/* Captain Info for Delivery */}
              {getTableName(selectedOrder).toLowerCase() === 'delivery' || 
               getTableName(selectedOrder).toLowerCase() === 'توصيل' ? (
                <Box>
                  <Text fontWeight="bold">Delivery Captain:</Text>
                  {selectedOrder.attributes?.captain?.data?.attributes ? (
                    <>
                      <Text>Name: {selectedOrder.attributes.captain.data.attributes.name}</Text>
                      <Text>Phone: {selectedOrder.attributes.captain.data.attributes.phone || "N/A"}</Text>
                      <Text>Auth Code: {selectedOrder.attributes.captain.data.attributes.authCode || "N/A"}</Text>
                    </>
                  ) : (
                    <Text>No captain assigned</Text>
                  )}
                </Box>
              ) : null}
              ...
              <Box>
                ...
                {getOrderItems(selectedOrder).length === 0 ? (
                  ...
                  getOrderItems(selectedOrder).map((item, index) => (
                    <Box key={item.id || index} p={2} borderWidth="1px" borderRadius="md" mt={2}>
                      <HStack justify="space-between">
                        <VStack align="start" spacing={0}>
                          <Text fontWeight="medium">
                            {item.attributes?.quantity || item.quantity || 1}x {
                              item.attributes?.menu_item?.data?.attributes?.name || 
                              (item.menu_item?.name) || 'Unknown Item'
                            }
                          </Text>
                          <Text fontSize="sm">
                            ${item.attributes?.unit_price || item.unit_price || '0'} x {item.attributes?.quantity || item.quantity || 1}
                          </Text>
                          ...
                          {(item.attributes?.preparation_notes || item.preparation_notes) && (
                            <Text fontSize="sm" color="blue.600">
                              Prep: {item.attributes?.preparation_notes || item.preparation_notes}
                            </Text>
                          )}
                          {(parseFloat(item.attributes?.discount_amount || item.discount_amount || 0) > 0) && (
                            <Text fontSize="sm" color="green.600">
                              Discount: ${item.attributes?.discount_amount || item.discount_amount} 
                              ({item.attributes?.discount_reason || item.discount_reason || 'No reason'})
                            </Text>
                          )}
                        </VStack>
                        <Text fontWeight="bold">${item.attributes?.subtotal || item.subtotal || '0.00'}</Text>
                      </HStack>
                      {(item.attributes?.status || item.status) && (
                        <Badge mt={1} colorScheme={getOrderStatusColor(item.attributes?.status || item.status)}>
                          {item.attributes?.status || item.status}
                        </Badge>
                      )}
                    </Box>
              ...
              <Box>
                <HStack justify="space-between">
                  ...
                  <Text>${selectedOrder.attributes?.total || selectedOrder.total || '0.00'}</Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontWeight="bold">Payment Method:</Text>
                  <Text>{selectedOrder.attributes?.payment_method || selectedOrder.payment_method || 'N/A'}</Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontWeight="bold">Status:</Text>
                  <Badge
                    colorScheme={getOrderStatusColor(selectedOrder.attributes?.status || selectedOrder.status || 'pending')}
                    rounded="full"
                    px={2}
                  >
              ...
              <Box>
                ...
                <Text>{new Date(selectedOrder.attributes?.createdAt || selectedOrder.createdAt).toLocaleString()}</Text>
              </Box>
              ...
              <SimpleGrid columns={2} spacing={4}>
                ...
                {(selectedOrder.attributes?.status || selectedOrder.status) === 'ready' && (
                  <Button 
                    colorScheme="green" 
                ...
                {['pending', 'preparing', 'ready'].includes(selectedOrder.attributes?.status || selectedOrder.status) && (
                  <Button 
                    colorScheme="red" 
