...
const AgentDepositForm = ({ 
  ...
  const depositMutation = useMutation({
    mutationFn: async ({ amount, pin }) => {
        ...
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/transactions`, 
        {
          ...
          body: JSON.stringify({
            data: {
              ...
              currency: 'LYD',
              status: 'completed',
              sender: agentData.data.attributes.wallet.data.id,
              receiver: localCustomerData.attributes.wallet.data.id,
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error?.message || 'Deposit failed');
      }
    ...
    onSuccess: () => {
      ...
      if (onComplete) onComplete('deposit');
  ...
  const handleSubmit = (e) => {
    ...
    depositMutation.mutate({ amount, pin });
  };
          ...
          <VStack spacing={6}>
            ...
            <HStack w="full" spacing={4} pt={4}>
              ...
              <Button
                ...
                isDisabled={!amount || pin.length !== 6}
              >
                Confirm Deposit
              </Button>
            </HStack>
...
export default AgentDepositForm;